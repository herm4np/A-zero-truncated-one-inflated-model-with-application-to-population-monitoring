...
rztnb <- function(n, lambda, k) {
  y <- rnbinom(n, size = lambda*k, prob = 1/(1 + k))
  y <- y[y!=0]
  return(y)
}
...
llztoinb <- function(param, data = y) {
  ...
  prob <- 1/(1 + param[2]/(1 - p)) # 1/(1 + k/(1 - p))
  omega <- 1/(1 + lambda*p)
...
llztnb <- function(param, data = y) {
  ...
  prob <- 1/(1 + param[2]) # 1/(1 + k/(1 - p)), p = 0
...
ztoinb_ml_fit <- function(data = y, param = c(2, 2, 0.5)) {
  ml <- optim(par = param, fn = opt_llztoinb, data = data, hessian = TRUE)
  return(list(par = ml$par, hess = ml$hessian))
}

ztnb_ml_fit <- function(data = y, param = c(2, 2)) {
  ml <- optim(par = param, fn = opt_llztnb, data = data, hessian = TRUE, method = "L-BFGS-B") # use method to allow limits
  return(list(par = ml$par, hess = ml$hessian))
}

zotnb_ml_fit <- function(data = y, param = c(2, 2)) {
  ml <- optim(par = param, fn = opt_llzotnb, data = data, hessian = TRUE, method = "L-BFGS-B") # use method to allow limits
  return(list(par = ml$par, hess = ml$hessian))
}
...
cut <- sim_ztoinb(1000, 2, 1, 0.1, 10)
...
N_ztoinb <- function(param, data = y) {
  ...
  size <- lambda*param[2] #lambda * k
  prob <- 1 - 1/(1 + param[2]/(1 - p)) # 1/(1 + k/(1 - p))
  n <- length(data) - (sum(data==1)*p*lambda/(p*lambda + dnbinom(1, size, prob)) %>% round()) #subtract estimated proportion of "ghosts"
  N <- (n/(1 - dnbinom(0, size, prob))) %>%
    round()
  return(N)
}
...
# 
# set.seed(6556485)
# NB_SIM <- sim_nb(c(500,900,1300), c(0.5,1.75,3), c(1,3,5),  c(0, 0.1, 0.2), 100)
# 
# NB_SIM %>%
#   filter(lambda != 0.5) %>%
#   group_by(lambda, p, k) %>%
